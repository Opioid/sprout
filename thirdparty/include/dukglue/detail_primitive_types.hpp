...
#define DUKGLUE_SIMPLE_VALUE_TYPE(TYPE, DUK_IS_FUNC, DUK_GET_FUNC, DUK_PUSH_FUNC, PUSH_VALUE) \
		...
		struct DukType<TYPE> { \
			...
			\
			template<typename FullT> \
			static void push(duk_context* ctx, duk_uint_t class_idx, TYPE value) { \
		...
		DUKGLUE_SIMPLE_VALUE_TYPE(std::string_view, duk_is_string, duk_get_string, duk_push_string, value.data())
