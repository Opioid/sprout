...
void centered_squared_magnitude(float* result, float2 const* source, int32_t width, int32_t height);
...
void create(thread::Pool& pool) {
    ...
    if (near_field) {
        ...
        pool.run_range(
            [&float_image_a, &signal](uint32_t /*id*/, int32_t begin, int32_t end) {
           ...
           resolution);
                        ...
                        pool.run_range([spectral_data, &float_image_a](int32_t begin, int32_t end) {
                                for (int32_t i = begin; i < end; ++i) {
                                        ...
                                        float3 linear_rgb =
           spectrum::XYZ_to_linear_RGB(s.normalized_XYZ()); float_image_a.store(i,
           packed_float3(linear_rgb));
                                }
                        }, 0, resolution * resolution);
        ...
        pool.run_range(
            [spectral_data, &signal, resolution](uint32_t /*id*/, int32_t begin, int32_t end) {
...
void render_aperture(const Aperture& aperture, Float1& signal) {
    ...
    for (int32_t y = 0; y < resolution; ++y) {
        for (int32_t x = 0; x < resolution; ++x) {
            ...
            float a = 0.f;
            for (auto k : kernel) {
                float2 p(-1.f + 2.f * ((fx + k[0]) / fr), 1.f - 2.f * ((fy + k[1]) / fr));
...
void centered_squared_magnitude(float* result, float2 const* source, int32_t width,
                                int32_t height) {
