...
inline vec3 sample_hemisphere_uniform(float2 uv) {
	float z = 1.f - uv.x;
	...
	return vec3(cos_phi * r, sin_phi * r, z);
}

inline vec3 sample_oriented_hemisphere_uniform(float2 uv, const vec3& x, const vec3& y, const vec3& z) {
	float za = 1.f - uv.x;
...
inline vec3 sample_hemisphere_cosine(float2 uv) {
...
inline vec3 sample_oriented_hemisphere_cosine(float2 uv, const vec3& x, const vec3& y, const vec3& z) {
	float2 xy = sample_disk_concentric(uv);
...
inline vec3 sample_sphere_uniform(float2 uv) {
...
inline vec3 sample_oriented_cone_uniform(float2 uv, float cos_theta_max,
