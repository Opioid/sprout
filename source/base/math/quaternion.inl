...
void set_rotation(Quaternion<T>& q, T yaw, T pitch, T roll)
{
	...
	const T cos_yaw   = cos(half_yaw);
	const T sin_yaw   = sin(half_yaw);
	const T cos_pitch = cos(half_pitch);
	const T sin_pitch = sin(half_pitch);
	const T cos_roll  = cos(half_roll);
	const T sin_roll  = sin(half_roll);
