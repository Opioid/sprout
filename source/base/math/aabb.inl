...
inline f_float3 AABB::min() const { return bounds[0]; }

inline f_float3 AABB::max() const { return bounds[1]; }

inline float3 AABB::position() const { return 0.5f * (bounds[0] + bounds[1]); }

inline float3 AABB::halfsize() const { return 0.5f * (bounds[1] - bounds[0]); }

inline float3 AABB::extent() const { return bounds[1] - bounds[0]; }
