...
static inline float4 sRGB_to_linear_RGB(byte4 c) {
  return float4(sRGB_to_linear(static_cast<float>(c[0]) / 255.f),
                sRGB_to_linear(static_cast<float>(c[1]) / 255.f),
                sRGB_to_linear(static_cast<float>(c[2]) / 255.f), static_cast<float>(c[3]) / 255.f);
}
...
static inline constexpr float watt_to_lumen(float3 const& c) { return CIE_constant * luminance(c); }
