...
math::float3 linear_RGB_to_sRGB(math::pfloat3 c);
math::float4 linear_RGB_to_sRGB(const math::float4& c);

// convert sRGB gamma color to sRGB linear color
float sRGB_to_linear(float c);

math::float3 sRGB_to_linear_RGB(math::byte3 c);
math::float3 sRGB_to_linear_RGB(math::pfloat3 c);

math::float4 sRGB_to_linear_RGB(math::byte4 c);

// convert linear color to gamma color
math::float3 linear_to_gamma(math::pfloat3 c, float gamma);

// convert gamma color to linear color
math::float3 gamma_to_linear_RGB(math::pfloat3 c, float gamma);
