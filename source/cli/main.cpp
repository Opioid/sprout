...
int main(int argc, char const* argv[]) {
  ...
  if (args.progressive) {
    ...
    logging::info("Total render time " + string::to_string(chrono::seconds_since(rendering_start)) +
                  " s");

    logging::info("Total elapsed time " + string::to_string(chrono::seconds_since(total_start)) +
                  " s");
  }
...
void log_memory_consumption(resource::Manager const& manager, take::Take const& take,
  ...
  size_t const renderer_num_bytes =
      take.view.pipeline.num_bytes() + take.view.camera->sensor().num_bytes() + rendering_num_bytes;
  ...
  size_t const total_num_bytes =
      image_num_bytes + material_num_bytes + mesh_num_bytes + renderer_num_bytes + scene_num_bytes;
