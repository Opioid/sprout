...
bool Prop_light::sample(f_float3 p, float time, Transformation const& transformation,
  ...
  if (!prop_->shape()->sample(part_, p, transformation, area, two_sided, sampler, sampler_dimension,
                              worker.node_stack(), result.shape)) {
    return false;
  }

  result.radiance =
      material->sample_radiance(result.shape.wi, result.shape.uv, area, time, filter, worker);
