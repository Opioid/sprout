...
void Sample::sample(sampler::Sampler& sampler, bxdf::Sample& result) const {
	...
	if (p < 0.4f) {
		...
		float3 flakes_fresnel;
		auto const flakes = flakes_.evaluate(result.wi, wo_, result.h,
...
void Sample::Base_layer::set(float3 const& color_a, float3 const& color_b,
...
bxdf::Result Sample::Base_layer::evaluate(f_float3 wi, f_float3 wo, float3 const& h,
...
void Sample::Flakes_layer::set(float3 const& ior, float3 const& absorption,
...
bxdf::Result Sample::Flakes_layer::evaluate(f_float3 wi, f_float3 wo, float3 const& h,
