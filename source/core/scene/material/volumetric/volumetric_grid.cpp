...
Material::Boxi Grid::volume_texture_space_bounds(Scene const& scene) const {
    auto const& desc = density_.description(scene);

    return {desc.offset(), desc.dimensions()};
}
...
float Grid::density(float3_p uvw, Filter filter, Worker& worker) const {
    ...
    if (Filter::Nearest == filter) {
        ...
        return sampler.stochastic_1(density_, uvw, r, worker.scene());
    } else {
...
float3 Grid_emission::evaluate_radiance(float3_p /*wi*/, float3_p uvw, float /*volume*/,
    ...
    if (Filter::Nearest == filter) {
        ...
        float3 const emission = temperature_.is_valid() ? blackbody_(sampler.stochastic_1(
                                                              temperature_, uvw, r, worker.scene()))
                                                        : emission_;

        if (2 == density_.num_channels()) {
            float2 const d = sampler.stochastic_2(density_, uvw, r, worker.scene());

            return (d[0] * d[1]) * a_norm_ * emission;
        } else {
            float const d = sampler.stochastic_1(density_, uvw, r, worker.scene());

            return d * a_norm_ * emission;
        }
    } else {
        float3 const emission = temperature_.is_valid() ? blackbody_(sampler.sample_1(
                                                              temperature_, uvw, worker.scene()))
                                                        : emission_;
...
CCE Grid_color::collision_coefficients_emission(float3_p uvw, Filter filter, Worker& worker) const {
    float4 const c = color(uvw, filter, worker);
