...
const material::Sample& Glass_rough::sample(f_float3 wo, Renderstate const& rs,
...
float Glass_rough::ior() const { return ior_; }

bool Glass_rough::is_scattering_volume() const { return false; }

size_t Glass_rough::num_bytes() const { return sizeof(*this); }

void Glass_rough::set_normal_map(Texture_adapter const& normal_map) { normal_map_ = normal_map; }

void Glass_rough::set_roughness_map(Texture_adapter const& roughness_map) {
  roughness_map_ = roughness_map;
}

void Glass_rough::set_refraction_color(float3 const& color) { refraction_color_ = color; }
...
void Glass_rough::set_ior(float ior) { ior_ = ior; }
...
size_t Glass_rough::sample_size() { return sizeof(Sample_rough); }
