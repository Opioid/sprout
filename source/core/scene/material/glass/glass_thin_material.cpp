...
const material::Sample& Glass_thin::sample(f_float3 wo, Renderstate const& rs,
...
float3 Glass_thin::thin_absorption(f_float3 wo, f_float3 n, float2 uv, float time,
  ...
  float3 const attenuation = rendering::attenuation(approximated_distance, absorption_coefficient_);

  return opacity(uv, time, filter, worker) * (1.f - refraction_color_ * attenuation);
}

float Glass_thin::ior() const { return ior_; }

bool Glass_thin::has_tinted_shadow() const { return true; }

bool Glass_thin::is_scattering_volume() const { return false; }

size_t Glass_thin::num_bytes() const { return sizeof(*this); }

void Glass_thin::set_normal_map(Texture_adapter const& normal_map) { normal_map_ = normal_map; }

void Glass_thin::set_refraction_color(float3 const& color) { refraction_color_ = color; }
...
void Glass_thin::set_ior(float ior) { ior_ = ior; }

void Glass_thin::set_thickness(float thickness) { thickness_ = thickness; }

size_t Glass_thin::sample_size() { return sizeof(Sample_thin); }
