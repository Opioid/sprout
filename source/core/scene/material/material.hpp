...
public:
	...
	virtual math::float3 sample_emission(math::float2 uv, const image::texture::sampler::Sampler_2D& sampler) const = 0;
...
public:
	...
	Material(Sample_cache<T>& cache, std::shared_ptr<image::texture::Texture_2D> mask) : IMaterial(mask), cache_(cache) {}
