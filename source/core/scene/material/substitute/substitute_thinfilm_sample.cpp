...
math::float3 Sample_thinfilm::evaluate(math::pfloat3 wi, float& pdf) const {
	...
	if (0.f == thinfilm_.a2) {
		...
		float cl_clamped_a2 = ggx::clamp_a2(thinfilm_.a2);
...
void Sample_thinfilm::sample_evaluate(sampler::Sampler& sampler, bxdf::Result& result) const {
	...
	if (p < 0.5f) {
		...
		float n_dot_wi = specular.init_importance_sample(n_dot_wo, thinfilm_.a2, *this,
														 sampler, result);
		...
		float c_pdf;
		math::float3 c_reflection = specular.evaluate(n_dot_wi, n_dot_wo, thinfilm_.a2,
