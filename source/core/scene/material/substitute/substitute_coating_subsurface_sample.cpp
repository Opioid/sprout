...
void Sample_coating_subsurface::sample(Sampler& sampler, RNG& rng, bxdf::Sample& result) const {
    ...
    if (same_side) {
        ...
        if (p <= cf) {
            ...
            if (p <= f) {
                float const n_dot_wi = ggx::Iso::reflect(wo_, h, n_dot_wo, n_dot_h, wi_dot_h,
                                                         wo_dot_h, alpha_[0], layer, result);
                ...
                result.reflection = reflection * ggx::ilm_ep_conductor(base_.f0_, n_dot_wo,
                                                                       alpha_[0], base_.metallic_);
                result.pdf *= f;
            } else {
