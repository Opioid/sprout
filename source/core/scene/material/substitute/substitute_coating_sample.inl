...
bxdf::Result Sample_coating<Coat, Diff>::evaluate(float3_p wi) const {
    ...
    auto const coating = coating_.evaluate(wi, wo_, h, wo_dot_h, avoid_caustics());

    bool const pm = 1.f == base_.metallic_;

    auto const base = pm ? base_.pure_gloss_evaluate(wi, wo_, h, wo_dot_h, *this)
                         : base_.base_evaluate(wi, wo_, h, wo_dot_h, *this);
...
void Sample_coating<Coat, Diff>::coating_reflect(float f, float n_dot_h, Sample& result) const {
    ...
    auto const base = pm ? base_.pure_gloss_evaluate(result.wi, wo_, result.h, result.h_dot_wi,
                                                     *this)
                         : base_.base_evaluate(result.wi, wo_, result.h, result.h_dot_wi, *this);
