...
static inline float G(float3 const& view, float3 const& light, float3 const& half,
	...
	float const ra = (2.f * abs_lvz * abs_llz) / (math::length(alpha1 * lview) * abs_llz + math::length(alpha1 * llight) * abs_lvz);
...
float Isotropic::reflect_internally(f_float3 wo, float n_dot_wo, Layer const& layer,
...
template<typename Layer, typename Fresnel>
float Isotropic::refract(f_float3 wo, float n_dot_wo, Layer const& layer,
						 Fresnel const& fresnel, sampler::Sampler& sampler, bxdf::Sample& result) {
...
//			std::abs((eta * eta * Dot(wi, wh)) / (sqrtDenom * sqrtDenom));
	...
	//	float const oslpdf =  (std::abs(cosBeta * cosAlpha) * (sqr_eta) * (g1a * d) * wo_dot_h) / n_dot_wo;
...
template<typename Layer, typename Fresnel>
bxdf::Result Anisotropic::reflection(f_float3 h, float n_dot_wi, float n_dot_wo,
...
template<typename Layer, typename Fresnel>
float Anisotropic::reflect(f_float3 wo, float n_dot_wo,
						   Layer const& layer, Fresnel const& fresnel,
						   sampler::Sampler& sampler, bxdf::Sample& result) {
	float2 const xi = sampler.generate_sample_2D();
