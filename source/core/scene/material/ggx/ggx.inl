...
static inline float clamp_roughness(float roughness) { return std::max(roughness, Min_roughness); }
...
float Isotropic::refract(f_float3 wo, float n_dot_wo, Layer const& layer, IOR const& ior,
  ...
  /*
          // Roughness zero will always have zero specular term (or worse NaN)
                  ...
                  // orthonormal basis
                  float3 const cross_v_z = float3(v[1], -v[0], 0.f); // == cross(v, [0, 0, 1])
                  float3 const t1 = (v[2] < 0.9999f) ? math::normalize(cross_v_z) : float3(1.f, 0.f,
  0.f);
                  // cross(t1, v);
