...
Result Clearcoat::evaluate(f_float3 wi, f_float3 wo, float3 const& h,
...
void Clearcoat::sample(f_float3 wo, float /*internal_ior*/,
	...
	float const n_dot_wi = ggx::Isotropic::reflect(wo, n_dot_wo, layer, schlick,
...
template<typename Layer>
Result Thinfilm::evaluate(f_float3 wi, f_float3 wo, float3 const& h,
						  float wo_dot_h, float internal_ior, Layer const& layer) const {
	float const n_dot_wi = layer.clamp_n_dot(wi);
	float const n_dot_wo = layer.clamp_abs_n_dot(wo);
...
void Thinfilm::sample(f_float3 wo, float internal_ior, Layer const& layer,
	...
	float const n_dot_wi = ggx::Isotropic::reflect(wo, n_dot_wo, layer, thinfilm,
												   sampler, attenuation, result);
...
Result Coating_layer<Coating>::evaluate(f_float3 wi, f_float3 wo, float3 const& h,
