...
inline void Clearcoat::set(float3 const& absorption_coefficient, float thickness, float ior,
                           float f0, float alpha, float weight) noexcept {
...
inline Result Clearcoat::evaluate(float3 const& wi, float3 const& wo, float3 const& h,
...
inline void Clearcoat::sample(float3 const& wo, Layer const& layer, sampler::Sampler& sampler,
...
inline Result Thinfilm::evaluate(float3 const& wi, float3 const& wo, float3 const& h,
                                 float wo_dot_h, Layer const& layer, bool /*avoid_caustics*/) const
    noexcept {
    float const n_dot_wi = layer.clamp_n_dot(wi);
    float const n_dot_wo = layer.clamp_abs_n_dot(wo);
...
inline void Thinfilm::sample(float3 const& wo, Layer const& layer, sampler::Sampler& sampler,
                             float3& attenuation, bxdf::Sample& result) const noexcept {
    float const n_dot_wo = layer.clamp_abs_n_dot(wo);
