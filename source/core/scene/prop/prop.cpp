...
//ray_max_t, 					   float ray_time, shape::Node_stack& node_stack) const { 	if (!visible_in_shadow()) {
//		return false;
//	}
...
const shape::Shape* Prop::shape() const { return shape_.get(); }

shape::Shape* Prop::shape() { return shape_.get(); }

math::AABB const& Prop::aabb() const { return aabb_; }
...
float Prop::area(uint32_t part) const { return parts_[part].area; }

uint32_t Prop::light_id(uint32_t part) const { return parts_[part].light_id; }

material::Material* Prop::material(uint32_t part) const { return materials_[part].get(); }

bool Prop::has_masked_material() const { return properties_.test(Property::Masked_material); }

bool Prop::has_tinted_shadow() const { return properties_.test(Property::Tinted_shadow); }

bool Prop::has_no_surface() const { return 1 == materials_.size() && 1.f == materials_[0]->ior(); }

size_t Prop::num_bytes() const { return sizeof(*this); }
