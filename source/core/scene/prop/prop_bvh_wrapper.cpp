...
bvh::Tree<Prop>& BVH_wrapper::tree() { return tree_; }
...
math::AABB const& BVH_wrapper::aabb() const { return tree_.aabb_; }
    ...
    if (node.intersect_p(ray_origin, ray_inv_direction, ray_min_t, ray_max_t)) {
      ...
      for (uint32_t i = node.indices_start(), len = node.indices_end(); i < len; ++i) {
        if (props[i]->intersect_p(ray, node_stack)) {
          ...
          //ray_inv_direction, 										  ray_min_t, ray_max_t, ray.time, node_stack)) {
          return true;
        }
      }
    }
