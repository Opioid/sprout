...
inline void Norm23x2_Sign1x2::decode(math::packed_float3& v, float& s) const {
	...
	const uint32_t c = (static_cast<uint32_t>(data[2]) & 0x00003FFF) << 9;
	const uint32_t y = b | c;

	const uint32_t d = (static_cast<uint32_t>(data[2]) >> 14) & 0x00000001;
	const uint32_t e = (static_cast<uint32_t>(data[2]) >> 15) & 0x00000001;
...
inline void Norm23x2_Sign1x2::encode(const math::packed_float3& v, float s) {
	...
	const uint32_t zs = v.z >= 0.f ? 0 : 1;
	const uint32_t is = s   >= 0.f ? 0 : 1;
