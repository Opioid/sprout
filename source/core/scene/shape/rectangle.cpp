...
Rectangle::Rectangle() { aabb_.set_min_max(float3(-10.f, -10.f, -0.1f), float3(10.f, 10.f, 0.1f)); }
...
bool Rectangle::sample(uint32_t /*part*/, f_float3 p, Transformation const& transformation,
  ...
  float3 ws = transformation.position + math::transform_vector(scale * ls, transformation.rotation);
...
float Rectangle::uv_weight(float2 /*uv*/) const { return 1.f; }

float Rectangle::area(uint32_t /*part*/, f_float3 scale) const { return 4.f * scale[0] * scale[1]; }

size_t Rectangle::num_bytes() const { return sizeof(*this); }
