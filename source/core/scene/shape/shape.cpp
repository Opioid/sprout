...
math::AABB const& Shape::aabb() const { return aabb_; }

float3 Shape::object_to_texture_point(f_float3 p) const {
  return (p - aabb_.bounds[0]) * inv_extent_;
}

float3 Shape::object_to_texture_vector(f_float3 v) const { return v * inv_extent_; }
...
uint32_t Shape::num_parts() const { return 1; }
...
bool Shape::is_complex() const { return false; }

bool Shape::is_finite() const { return true; }

bool Shape::is_analytical() const { return true; }

void Shape::prepare_sampling(uint32_t /*part*/) {}

Morphable_shape* Shape::morphable_shape() { return nullptr; }
