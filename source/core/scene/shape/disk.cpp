...
Disk::Disk() { aabb_.set_min_max(float3(-1.f, -1.f, -0.1f), float3(1.f, 1.f, 0.1f)); }
...
float Disk::uv_weight(float2 /*uv*/) const { return 1.f; }

float Disk::area(uint32_t /*part*/, f_float3 scale) const { return math::Pi * scale[0] * scale[0]; }

size_t Disk::num_bytes() const { return sizeof(*this); }
