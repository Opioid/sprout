...
math::AABB const& Scene::aabb() const { return prop_bvh_.aabb(); }
...
float Scene::tick_duration() const { return static_cast<float>(tick_duration_); }

float Scene::simulation_time() const { return static_cast<float>(simulation_time_); }

uint64_t Scene::current_tick() const { return current_tick_; }
...
std::vector<light::Light*> const& Scene::lights() const { return lights_; }
...
bool Scene::has_volumes() const { return has_volumes_; }
...
void Scene::add_material(Material_ptr const& material) { materials_.push_back(material); }
