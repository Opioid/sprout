...
void EMS::generate_camera_sample(int2 pixel, uint32_t index, Camera_sample& sample) {
	float2 s2d = math::ems(index, scramble_.v[0], scramble_.v[1]);

	sample.pixel = pixel;
	sample.pixel_uv = s2d;
	sample.lens_uv = s2d.yx();
	sample.time = math::radical_inverse_vdC(index, scramble_.v[1]);
}

float2 EMS::generate_sample_2D(uint32_t /*dimension*/) {
	return math::ems(current_sample_2D_[0]++, scramble_.v[0], scramble_.v[1]);
}
