...
void normalize() {
    ...
    for (size_t i = 0; i < num_values; ++i) {
        vectors[i] = float3(5.f - (10.f * rng.random_float()), 5.f - (10.f * rng.random_float()),
...
struct alignas(16) Struct_vector {
    ...
    Struct_vector() {}
    Struct_vector(float x, float y, float z)
        : x(x),
          y(y),
          z(z)  //, w(0.f)
    {
...
void test_union_vector(Union_vector* uvecs, size_t num_values) {
    ...
    auto const duration = chrono::seconds_since(start);
    std::cout << "[" << result.x << ", " << result.y << ", " << result.z << "] in "
              << string::to_string(duration) << " s" << std::endl;
}
...
void test_array_vector(Array_vector* vecs, size_t num_values) {
    ...
    auto const duration = chrono::seconds_since(start);
    std::cout << "[" << result[0] << ", " << result[1] << ", " << result[2] << "] in "
              << string::to_string(duration) << " s" << std::endl;
}
...
void basis() {
    ...
    //			auto const duration = chrono::seconds_since(start);
    //			std::cout << result << " in " << string::to_string(duration) << " s" <<
