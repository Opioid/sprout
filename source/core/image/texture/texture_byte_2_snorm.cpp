...
Byte2_snorm::Byte2_snorm(const std::shared_ptr<Image>& image) :
	Texture(image),
...
float4 Byte2_snorm::gather_1(const int4& xy_xy1) const {
	byte2 v[4];
...
void Byte2_snorm::gather_2(const int4& xy_xy1, float2 c[4]) const {
	byte2 v[4];
	image_.gather(xy_xy1, v);
...
void Byte2_snorm::gather_3(const int4& xy_xy1, float3 c[4]) const {
	byte2 v[4];
	image_.gather(xy_xy1, v);
