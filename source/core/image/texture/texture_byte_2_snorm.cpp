...
Byte2_snorm::Byte2_snorm(std::shared_ptr<Image> const& image)
...
float4 Byte2_snorm::gather_1(int4 const& xy_xy1) const {
    byte2 v[4];
...
float4 Byte2_snorm::at_element_4(int32_t x, int32_t y, int32_t element) const {
    auto const value = image_.load_element(x, y, element);
    return float4(encoding::cached_snorm_to_float(value[0]),
                  encoding::cached_snorm_to_float(value[1]), 0.f, 1.f);
}
