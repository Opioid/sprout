...
math::vec3 Texture_2D_byte_2_unorm::at_3(int32_t x, int32_t y) const {
	auto& value = image_.at(x, y);
	return math::vec3(encoding::unorm_to_float(value.x), encoding::unorm_to_float(value.y), 0.f);
}
...
math::vec3 Texture_2D_byte_2_unorm::at_3(int32_t x, int32_t y, int32_t element) const {
	auto& value = image_.at(x, y, element);
	return math::vec3(encoding::unorm_to_float(value.x), encoding::unorm_to_float(value.y), 0.f);
}
