...
float Nearest_3D<Address_mode>::sample_1(Texture const& texture, f_float3 uvw) const noexcept {
...
template <typename Address_mode>
float2 Nearest_3D<Address_mode>::sample_2(Texture const& texture, f_float3 uvw) const noexcept {
    const int3 xyz = map(texture, uvw);
...
template <typename Address_mode>
float3 Nearest_3D<Address_mode>::sample_3(Texture const& texture, f_float3 uvw) const noexcept {
    const int3 xyz = map(texture, uvw);
...
template <typename Address_mode>
float3 Nearest_3D<Address_mode>::address(f_float3 uvw) const noexcept {
    return float3(Address_mode::f(uvw[0]), Address_mode::f(uvw[1]), Address_mode::f(uvw[2]));
}

template <typename Address_mode>
int3 Nearest_3D<Address_mode>::map(Texture const& texture, f_float3 uvw) noexcept {
