...
Texture_2D_byte_1_unorm::Texture_2D_byte_1_unorm(std::shared_ptr<Image> image) :
	Texture_2D(image),
	data_(reinterpret_cast<const unsigned char*>(image->data())) {}

float Texture_2D_byte_1_unorm::at_1(uint32_t x, uint32_t y) const {
...
math::float2 Texture_2D_byte_1_unorm::at_2(uint32_t x, uint32_t y) const {
	uint32_t i = y * dimensions().x + x;
...
math::float3 Texture_2D_byte_1_unorm::at_3(uint32_t x, uint32_t y) const {
	uint32_t i = y * dimensions().x + x;
...
math::float4 Texture_2D_byte_1_unorm::at_4(uint32_t x, uint32_t y) const {
	uint32_t i = y * dimensions().x + x;
