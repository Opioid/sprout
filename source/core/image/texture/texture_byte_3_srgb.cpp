...
Byte3_sRGB::Byte3_sRGB(std::shared_ptr<Image> image) :
	Texture(image),
...
float4 Byte3_sRGB::gather_1(const int4& xy_xy1) const {
	byte3 v[4];
	image_.gather(xy_xy1, v);

	return float4(encoding::cached_srgb_to_float(v[0][0]),
				  encoding::cached_srgb_to_float(v[1][0]),
				  encoding::cached_srgb_to_float(v[2][0]),
				  encoding::cached_srgb_to_float(v[3][0]));
}

void Byte3_sRGB::gather_2(const int4& xy_xy1, float2 c[4]) const {
	byte3 v[4];
	image_.gather(xy_xy1, v);

	c[0] = float2(encoding::cached_srgb_to_float(v[0][0]),
				  encoding::cached_srgb_to_float(v[0][1]));

	c[1] = float2(encoding::cached_srgb_to_float(v[1][0]),
				  encoding::cached_srgb_to_float(v[1][1]));

	c[2] = float2(encoding::cached_srgb_to_float(v[2][0]),
				  encoding::cached_srgb_to_float(v[2][1]));

	c[3] = float2(encoding::cached_srgb_to_float(v[3][0]),
				  encoding::cached_srgb_to_float(v[3][1]));
}

void Byte3_sRGB::gather_3(const int4& xy_xy1, float3 c[4]) const {
/*	byte3 v[4];
	image_.gather(xy_xy1, v);
...
*/
	...
	c[0] = float3(encoding::cached_srgb_to_float(v0[0]),
				  encoding::cached_srgb_to_float(v0[1]),
				  encoding::cached_srgb_to_float(v0[2]));

	const byte3 v1 = image_.load(y0 + xy_xy1[2]);
	c[1] = float3(encoding::cached_srgb_to_float(v1[0]),
				  encoding::cached_srgb_to_float(v1[1]),
				  encoding::cached_srgb_to_float(v1[2]));

	const int32_t y1 = width * xy_xy1[3];

	const byte3 v2 = image_.load(y1 + xy_xy1[0]);
	c[2] = float3(encoding::cached_srgb_to_float(v2[0]),
				  encoding::cached_srgb_to_float(v2[1]),
				  encoding::cached_srgb_to_float(v2[2]));

	const byte3 v3 = image_.load(y1 + xy_xy1[2]);
	c[3] = float3(encoding::cached_srgb_to_float(v3[0]),
				  encoding::cached_srgb_to_float(v3[1]),
...
float4 Byte3_sRGB::at_element_4(int32_t x, int32_t y, int32_t element) const {
	auto value = image_.load_element(x, y, element);
	return float4(encoding::cached_srgb_to_float(value[0]),
				  encoding::cached_srgb_to_float(value[1]),
				  encoding::cached_srgb_to_float(value[2]),
