...
Float1::Float1(std::shared_ptr<Image> const& image)
    : Texture(image), image_(*reinterpret_cast<const image::Float1*>(image.get())) {}

float Float1::at_1(int32_t i) const {
...
float4 Float1::gather_1(int4 const& xy_xy1) const {
    float v[4];
...
void Float1::gather_2(int4 const& xy_xy1, float2 c[4]) const {
    float v[4];
    image_.gather(xy_xy1, v);
...
void Float1::gather_3(int4 const& xy_xy1, float3 c[4]) const {
    float v[4];
    image_.gather(xy_xy1, v);
