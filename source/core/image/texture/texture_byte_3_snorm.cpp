...
Byte3_snorm::Byte3_snorm(std::shared_ptr<Image> const& image)
    : Texture(image), image_(*static_cast<const Byte3*>(image.get())) {}

float Byte3_snorm::at_1(int32_t i) const {
...
float4 Byte3_snorm::gather_1(int4 const& xy_xy1) const {
    byte3 v[4];
    image_.gather(xy_xy1, v);

    return float4(
        encoding::cached_snorm_to_float(v[0][0]), encoding::cached_snorm_to_float(v[1][0]),
        encoding::cached_snorm_to_float(v[2][0]), encoding::cached_snorm_to_float(v[3][0]));
}

void Byte3_snorm::gather_2(int4 const& xy_xy1, float2 c[4]) const {
    byte3 v[4];
    image_.gather(xy_xy1, v);
    ...
    c[3] =
        float2(encoding::cached_snorm_to_float(v[3][0]), encoding::cached_snorm_to_float(v[3][1]));
}

void Byte3_snorm::gather_3(int4 const& xy_xy1, float3 c[4]) const {
    /*	byte3 v[4];
            image_.gather(xy_xy1, v);
