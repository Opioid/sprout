...
float Byte_3_unorm::at_1(int32_t x, int32_t y) const {
	...
	return encoding::cached_unorm_to_float(value.x);
}

float2 Byte_3_unorm::at_2(int32_t x, int32_t y) const {
	auto value = image_.load(x, y);
	return float2(encoding::cached_unorm_to_float(value.x),
				  encoding::cached_unorm_to_float(value.y));
}

float3 Byte_3_unorm::at_3(int32_t x, int32_t y) const {
	auto value = image_.load(x, y);
	return float3(encoding::cached_unorm_to_float(value.x),
				  encoding::cached_unorm_to_float(value.y),
				  encoding::cached_unorm_to_float(value.z));
}

float4 Byte_3_unorm::at_4(int32_t x, int32_t y) const {
	auto value = image_.load(x, y);
	return float4(encoding::cached_unorm_to_float(value.x),
				  encoding::cached_unorm_to_float(value.y),
				  encoding::cached_unorm_to_float(value.z),
				  1.f);
}
...
float4 Byte_3_unorm::gather_1(int4 xy_xy1) const {
	...
	return float4(encoding::cached_unorm_to_float(v00.x),
				  encoding::cached_unorm_to_float(v01.x),
				  encoding::cached_unorm_to_float(v10.x),
				  encoding::cached_unorm_to_float(v11.x));
}
...
void Byte_3_unorm::gather_2(int4 xy_xy1, float2 c[4]) const {
	...
	c[0] = float2(encoding::cached_unorm_to_float(v00.x),
				  encoding::cached_unorm_to_float(v00.y));

	c[1] = float2(encoding::cached_unorm_to_float(v01.x),
				  encoding::cached_unorm_to_float(v01.y));

	c[2] = float2(encoding::cached_unorm_to_float(v10.x),
				  encoding::cached_unorm_to_float(v10.y));

	c[3] = float2(encoding::cached_unorm_to_float(v11.x),
				  encoding::cached_unorm_to_float(v11.y));
}
...
void Byte_3_unorm::gather_3(int4 xy_xy1, float3 c[4]) const {
	...
	c[0] = float3(encoding::cached_unorm_to_float(v00.x),
				  encoding::cached_unorm_to_float(v00.y),
				  encoding::cached_unorm_to_float(v00.z));

	c[1] = float3(encoding::cached_unorm_to_float(v01.x),
				  encoding::cached_unorm_to_float(v01.y),
				  encoding::cached_unorm_to_float(v01.z));

	c[2] = float3(encoding::cached_unorm_to_float(v10.x),
				  encoding::cached_unorm_to_float(v10.y),
				  encoding::cached_unorm_to_float(v10.z));

	c[3] = float3(encoding::cached_unorm_to_float(v11.x),
				  encoding::cached_unorm_to_float(v11.y),
				  encoding::cached_unorm_to_float(v11.z));
}
