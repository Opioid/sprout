...
void Renderer::draw_disk(float2 pos, float3_p normal, float radius) {
    ...
    for (int32_t y = start[1]; y < end[1]; ++y) {
        for (int32_t x = start[0]; x < end[0]; ++x) {
            float2 const sample((float(x) + 0.5f) / dimensions_f_[0],
                                (float(y) + 0.5f) / dimensions_f_[1]);
...
void Renderer::draw_disk(float2 pos, float3_p normal, float radius, int32_t border) {
    ...
    float const padded_radius = radius + float(border) / dimensions_f_[0];
...
void Renderer::resolve_linear(Byte3& image) const {
    if (1 == sqrt_num_samples_) {
        for (int32_t i = 0, len = int32_t(image.description().num_pixels()); i < len; ++i) {
            auto const s = samples_[i];

            byte3 const srgb = encoding::float_to_unorm(s.xyz());
            image.store(i, srgb);
        }
    } else {
        int32_t const num_samples = sqrt_num_samples_ * sqrt_num_samples_;
        ...
        for (int32_t i_y = 0; i_y < i_d[1]; ++i_y) {
            ...
            for (int32_t i_x = 0; i_x < i_d[0]; ++i_x) {
                ...
                byte3 const srgb = encoding::float_to_unorm(n * result);
                image.store(i_x, i_y, srgb);
            }
        }
    }
...
bool Renderer::intersect_n_gon(float2 pos, float angle, float radius, uint32_t num_vertices,
    ...
    for (uint32_t i = 0; i < num_vertices; ++i) {
        float const delta = (2.f * Pi) / float(num_vertices);
        float const start = float(i) * delta + angle;

        auto const [sin_start, cos_start] = sincos(start);

        float2 const b(sin_start, cos_start);

        auto const [sin_start_delta, cos_start_delta] = sincos(start + delta);
