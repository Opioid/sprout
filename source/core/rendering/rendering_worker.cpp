...
void Worker::render_ex(uint32_t frame, uint32_t view, uint32_t iteration, int4 const& tile,
    ...
    uint32_t const num_initial_samples = 16;//std::min(16u, num_samples);// uint32_t(0.1f * float(num_samples));
    ...
    for (int32_t y = tile[1], y_back = tile[3]; y <= y_back; ++y) {
        ...
        sampler_->resize(num_initial_samples, 1, 2, 1);
            ...
            float const variance = m_newS / float(num_initial_samples - 1);
            sensor.set_variance(pixel, average > 0.f ? std::min(std::sqrt(variance / average), 1.f) : 0.f);
            //  sensor.set_variance(pixel, rendered_samples);



            float const guess = average > 0.f ? std::max(std::min(std::sqrt(variance / average), 1.f), 0.1f) : 0.f;

       uint32_t const remaining_samples = num_samples - num_initial_samples;



            uint32_t const new_samples = std::max(uint32_t(float(remaining_samples) * guess), 1u);
...
void Worker::render_a(uint32_t frame, uint32_t view, uint32_t iteration, int4 const& tile,
                    uint32_t num_samples) {
    Camera const& camera = *camera_;
    ...
    uint32_t const num_initial_samples = 16;//uint32_t(0.1f * float(num_samples));
...
void Worker::render_b(uint32_t frame, uint32_t view, uint32_t iteration, int4 const& tile,
                    uint32_t num_samples) {
    Camera const& camera = *camera_;
    ...
    uint32_t const num_initial_samples = num_samples - 16;
    ...
    for (int32_t y = tile[1], y_back = tile[3]; y <= y_back; ++y) {
        ...
        for (int32_t x = tile[0], x_back = tile[2]; x <= x_back; ++x) {
            ...
            uint32_t const num_actual_samples = std::max(uint32_t(variance * float(num_initial_samples)), 1u);



            surface_integrator_->prepare(*scene_, num_actual_samples);
        volume_integrator_->prepare(*scene_, num_actual_samples);


        sampler_->resize(num_actual_samples, 1, 2, 1);
