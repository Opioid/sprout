...
void Worker::init(uint32_t id, take::Settings const& settings, scene::Scene const& scene,
    ...
    if (photon_settings.num_photons) {
        integrator::photon::Mapper::Settings ps{
            photon_settings.max_bounces, photon_settings.max_photons_per_path,
            photon_settings.indirect_caustics, photon_settings.full_light_path};
