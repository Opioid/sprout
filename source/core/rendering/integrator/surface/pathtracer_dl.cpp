  ...
  for (uint32_t i = settings_.num_light_samples; i > 0; --i) {
    ...
    if (light.ref.sample(intersection.geo.p, material_sample.geometric_normal(), ray.time,
      ...
      if (math::any_greater_zero(tv)) {
        ...
        //!intersection.geo.subsurface) { 					std::cout << ray.length() << std::endl;
        //				}
...
size_t Pathtracer_DL::num_bytes() const { return sizeof(*this) + sampler_.num_bytes(); }
...
Pathtracer_DL_factory::~Pathtracer_DL_factory() { memory::free_aligned(integrators_); }
