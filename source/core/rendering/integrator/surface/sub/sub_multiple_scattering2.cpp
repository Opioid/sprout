#include "sub_multiple_scattering2.hpp"
...
Multiple_scattering2::Multiple_scattering2(rnd::Generator& rng, const take::Settings& take_settings/*,
...
void Multiple_scattering2::prepare(const Scene& /*scene*/, uint32_t num_samples_per_pixel) {
...
void Multiple_scattering2::resume_pixel(uint32_t sample, rnd::Generator& scramble) {
...
float3 Multiple_scattering2::li(const Ray& ray, Intersection& intersection,
	...
	for (uint32_t i = 0; /*i < 256*/; ++i) {
		...
		tray.max_t = -std::log(r) / average_extinction;
		...
		if (!hit) {
			float3 pdf = extinction * bssrdf.transmittance(tray.max_t);
	...
	//		result += tr * scattering_albedo * local_radiance;
			result += tr * scattering_albedo * local_radiance;
...
size_t Multiple_scattering2::num_bytes() const {
...
sampler::Sampler& Multiple_scattering2::material_sampler(uint32_t bounce, uint32_t iteration) {
...
Multiple_scattering2_factory::Multiple_scattering2_factory(const take::Settings& take_settings,
														 uint32_t num_integrators) :
	Factory(take_settings),
	integrators_(memory::allocate_aligned<Multiple_scattering2>(num_integrators)) {}

Multiple_scattering2_factory::~Multiple_scattering2_factory() {
	memory::free_aligned(integrators_);
}

Integrator* Multiple_scattering2_factory::create(uint32_t id, rnd::Generator& rng) const {
	return new(&integrators_[id]) Multiple_scattering2(rng, take_settings_/*, settings_*/);
}
