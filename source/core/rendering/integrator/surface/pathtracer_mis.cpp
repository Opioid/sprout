...
float3 Pathtracer_MIS::li(Ray& ray, Intersection& intersection, Worker& worker) {
  ...
  for (uint32_t i = ray.depth;; ++i) {
    ...
    result += throughput *
              sample_lights(ray, ray_offset, intersection, material_sample, do_mis, filter, worker);
    ...
    if (do_mis || treat_as_singular) {
      bool pure_emissive;
      float3 const radiance = evaluate_light(ray, intersection, sample_result, treat_as_singular,
                                             is_translucent, filter, worker, pure_emissive);
...
float3 Pathtracer_MIS::sample_lights(Ray const& ray, float ray_offset, Intersection& intersection,
  ...
  if (Light_sampling::Strategy::Single == settings_.light_sampling.strategy) {
    ...
    for (uint32_t l = 0, len = static_cast<uint32_t>(lights.size()); l < len; ++l) {
      ...
      for (uint32_t i = num_samples; i > 0; --i) {
        result += evaluate_light(light, light_weight, ray, ray_offset, l, do_mis, intersection,
                                 material_sample, filter, worker);
      }
    }
...
Pathtracer_MIS_factory::~Pathtracer_MIS_factory() { memory::free_aligned(integrators_); }
