...
float3 AO::li(Ray& ray, Intersection& intersection, Worker& worker) {
  ...
  auto const& material_sample =
      intersection.sample(wo, ray, Sampler_filter::Undefined, sampler_, worker);
...
size_t AO::num_bytes() const { return sizeof(*this) + sampler_.num_bytes(); }
...
AO_factory::~AO_factory() { memory::free_aligned(integrators_); }
