...
Pathtracer_NG::Pathtracer_NG(rnd::Generator& rng, const take::Settings& take_settings,
...
void Pathtracer_NG::prepare(const Scene& scene, uint32_t num_samples_per_pixel) {
...
size_t Pathtracer_NG::num_bytes() const {
...
sampler::Sampler& Pathtracer_NG::light_sampler(uint32_t bounce, Ray::Properties properties) {
...
Pathtracer_NG_factory::Pathtracer_NG_factory(const take::Settings& take_settings,
	...
	integrators_(memory::allocate_aligned<Pathtracer_NG>(num_integrators)),
	settings_{
		min_bounces,
		max_bounces,
...
Pathtracer_NG_factory::~Pathtracer_NG_factory() {
	memory::free_aligned(integrators_);
}

Integrator* Pathtracer_NG_factory::create(uint32_t id, rnd::Generator& rng) const {
	return new(&integrators_[id]) Pathtracer_NG(rng, take_settings_, settings_,
