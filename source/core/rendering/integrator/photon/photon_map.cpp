...
Map::Map(uint32_t num_photons, float search_radius, float merge_radius,
    ...
    : num_photons_(num_photons),
      ...
      direct_grid_(search_radius, 1.5f),
      indirect_grid_(indirect_radius_factor * search_radius, 1.1f) {}
...
uint32_t Map::compile_iteration(uint32_t num_paths, thread::Pool& pool) noexcept {
    ...
    if (separate_indirect_) {
        ...
        direct_grid_.init_cells(num_caustics, photons_);

        uint32_t const red_num_caustics = direct_grid_.reduce_and_move(photons_, merge_radius_,
                                                                       num_reduced_, pool);
        ...
        red_num_direct_   = red_num_caustics;
        red_num_indirect_ = red_num_indirect;

        return red_num_caustics + red_num_indirect;
    } else {
        direct_grid_.init_cells(num_photons_, photons_);
...
void Map::compile_finalize() noexcept {
    direct_grid_.init_cells(red_num_direct_, photons_);

    if (separate_indirect_) {
        indirect_grid_.init_cells(red_num_indirect_, photons_ + red_num_direct_);
    }
}

float3 Map::li(Intersection const& intersection, Material_sample const& sample,
               scene::Worker const& worker) const noexcept {
    return direct_grid_.li(intersection, sample, num_paths_, worker) +
...
size_t Map::num_bytes() const noexcept {
    ...
    num_bytes += direct_grid_.num_bytes() + indirect_grid_.num_bytes();
