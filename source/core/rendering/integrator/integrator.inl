...
template <typename B>
Pool<B>::Pool(uint32_t num_integrators) : num_integrators_(num_integrators) {}
...
Typed_pool<T, B>::Typed_pool(uint32_t num_integrators)
    : Pool<B>(num_integrators), integrators_(memory::allocate_aligned<T>(num_integrators)) {
...
Typed_pool<T, B>::~Typed_pool() {
    ...
    for (uint32_t i = 0, len = Typed_pool<T, B>::num_integrators_; i < len; ++i) {
