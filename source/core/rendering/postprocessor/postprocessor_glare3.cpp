...
void Glare3::init(scene::camera::Camera const& camera, thread::Pool& pool) {
        ...
        [this, dim, solid_angle, wl_norm, &CIE_X, &CIE_Y, &CIE_Z, &f, &inits](
            ...
            for (int32_t y = begin; y < end; ++y) {
                for (int32_t x = 0; x < kernel_dimensions_[0]; ++x) {
                    ...
                    float theta = length(float2(p)) * solid_angle;
    ...
    // Gaussian blur
    float const radius = 16.f;  // float(std::max(dim[0], dim[1])) * 0.00125f;
    ...
    const math::filter::Gaussian_functor gauss(float(fr * fr),
                                               /*radius * 0.5f*/ 0.15f);
...
void Glare3::apply(uint32_t id, uint32_t pass, int32_t begin, int32_t end,
    ...
    if (0 == pass) {
        ...
        float const weight = float(d[0] * d[1]) / float(num_samples);
