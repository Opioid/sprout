...
class Loader {
  public:
    ...
    static bool load(Take& take, std::istream& stream, std::string_view take_name, bool progressive, Scene& scene,
                     resource::Manager& resources) noexcept;

    static Camera* load_camera(json::Value const& camera_value, Scene& scene) noexcept;

    static void load_integrators(json::Value const& integrator_value, uint32_t num_workers, bool progressive,
                                 Take& take) noexcept;

    static void set_default_integrators(uint32_t num_workers, bool progressive, Take& take) noexcept;
